...
Users download a UTF-8 CSV template, upload their file, validate rows before any data is written, preview valid/warning/error lines, then import only valid rows into an existing quiz (MVP) or the question bank (later). Import runs in small batches with progress feedback so large files do not time out. Supported MVP question types align with LearnPress core: Single Choice, Multi Choice, True/False, and Fill-in-Blanks. Phase 2 adds flexible column mapping, import history, undo, export, images, categories, and extensibility hooks for custom question types.
...
Default new questions as **publish** so imported items appear immediately; authors can set status=draft explicitly for review. Design import column aliases and type slugs so CSV values map cleanly to LearnPress internal types (`single_choice`, `multi_choice`, `true_or_false`).
...
- Support LearnPress core question types: **single_choice**, **multi_choice**, **true_or_false**, **fill_in_blanks** (normalize aliases such as `true_false` → `true_or_false`, `multiple_choice` → `multi_choice`, `fib` → `fill_in_blanks`).
...
- Create questions as **publish** by default; store answers and correct flags correctly; append to selected quiz in CSV row order.
...
- **MVP:** CSV upload + template, import into one existing quiz, 4 LearnPress core types, fixed columns, validate + limited preview, publish status, batched import, error report.
