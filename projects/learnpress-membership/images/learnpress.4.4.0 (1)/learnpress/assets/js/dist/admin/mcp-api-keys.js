...
(function () {
  ...
  const onSubmitKey = () => {
    ...
    runRequest(dataSend, {
      success: response => {
        ...
        const message = response && response.message ? response.message : i18n.request_failed || 'Request failed.';
        if (status !== 'success') {
          setStatus(message, true);
          return;
        }
        setStatus(message, false);
        renderCredentials(response && response.data && response.data.key ? response.data.key : null);
        refreshKeysTable();
      },
  ...
  const onCopy = async elCopy => {
    const targetId = elCopy && elCopy.dataset && elCopy.dataset.target ? elCopy.dataset.target : '';
    if (!targetId) {
      return;
    }
